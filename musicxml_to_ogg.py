...
def musicxml_to_ogg(
    ...
    ogg = Path(output) if output else xml.with_suffix(".ogg")
    ...
    # ── 2단계: MIDI → OGG (fluidsynth CLI) ──────────────────────────────────
    print(f"[2/2] MIDI → OGG     : {mid} → {ogg}")
    cmd = [
        "fluidsynth",
...
def main():
    parser = argparse.ArgumentParser(
        description="MusicXML → OGG 변환 (music21 + fluidsynth CLI)"
    )
